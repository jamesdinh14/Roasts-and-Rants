namespace Roasts_and_Rants.Migrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<Roasts_and_Rants.DAL.RestaurantReviewContext>
    {
        ...
        protected override void Seed(Roasts_and_Rants.DAL.RestaurantReviewContext context)
        {
			...
			//addresses.ForEach(a => context.Addresses.AddOrUpdate(p => p.Street, a));
			//context.SaveChanges();

			//var reviews = new List<Review>() {
			//	new Review() { UserEmail = "Chase@example.com", RestaurantID = restaurants.Single(r => r.Name == "Five Guys").RestaurantID, Rating = 7.3M, Content = "Good burgers. Too greasy." },
			//	new Review() { UserEmail = "Chase@example.com", RestaurantID = restaurants.Single(r => r.Name == "Whole Foods").RestaurantID, Rating = 10.0M, Content = "It's organic." },
			//	new Review() { UserEmail = "James@nugget.com", RestaurantID = restaurants.Single(r => r.Name == "In-N-Out Burger").RestaurantID, Rating = 10.0M, Content = "Best fast food burgers EVAR!!" },
			//	new Review() { UserEmail = "John@hotmail.com", RestaurantID = restaurants.Single(r => r.Name == "Whole Foods").RestaurantID, Rating = 2.7M, Content = "Everything's too expensive." },
			//	new Review() { UserEmail = "PlasmaxTravis@gmail.com", RestaurantID = restaurants.Single(r => r.Name == "Five Guys").RestaurantID, Rating = 9.0M, Content = "Best burgers you could ask for. Fries are a little disappointing though." },
			//	new Review() { UserEmail = "Albert@Levin.saber", RestaurantID = restaurants.Single(r => r.Name == "Five Guys").RestaurantID, Rating = 9.0M, Content = "THUNDEROUS FURY!!" }
			//};

			//foreach (Review r in reviews) {
			//	var reviewInDatabase = context.Reviews.Where(u => u.User.Email == r.UserEmail
			//	&& u.Restaurant.RestaurantID == r.RestaurantID).SingleOrDefault();
